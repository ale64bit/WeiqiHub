...
class GradingExamPage extends StatefulWidget {
  const GradingExamPage(
  ...
  @override
  State<GradingExamPage> createState() => _GradingExamPageState();
}

const _timePerTask = Duration(seconds: 45);

class _GradingExamPageState extends State<GradingExamPage>
          ...
          child: Row(
            children: <Widget>[
              ...
              _SideBar(
                ...
                onNext: _onNext,
                onCancelExam: () {
                  context.stats.incrementGradingExamFailCount(widget.rank);
              ...
              onPressed: () {
                showDialog(
                  ...
                  builder: (context) => ConfirmDialog(
                    title: 'Confirm',
                    content:
                        'Are you sure that you want to stop the Grading Exam?',
                    onYes: () {
                      context.stats.incrementGradingExamFailCount(widget.rank);
                      Navigator.popUntil(context, (route) => route.isFirst);
                    },
  ...
  void _onNext() {
    if (_taskNumber == 10) {
      if (_mistakeCount <= 2) {
        context.stats.incrementGradingExamPassCount(widget.rank);
      } else {
        context.stats.incrementGradingExamFailCount(widget.rank);
      }
      showDialog(
                  ...
                  child: GradingExamPage(
                    ...
                    taskSource: ConstTaskSource(
                        tasks: TaskRepository()
                            .read(widget.rank, gradingExamTaskTypes, 10)),
                  ),
                ),
              ),
            );
                    ...
                    MaterialPageRoute(
                      builder: (context) => PopScope(
                        ...
                        child: GradingExamPage(
                          ...
                          taskSource: ConstTaskSource(
                              tasks: TaskRepository()
                                  .read(nextRank, gradingExamTaskTypes, 10)),
                        ),
                      ),
                    ),
                  );
